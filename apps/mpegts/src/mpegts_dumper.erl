...
dump_pes(Reader, PES) ->
  ...
  {ok, Reader1, Frames} = mpegts_reader:decode_pes(Reader, PES),
  Sorted = lists:sort(fun flv:frame_sorter/2, get(frame_buffer) ++ Frames),
  {Reply, Save} = if
    length(Sorted) > 4 -> lists:split(length(Sorted) - 4, Sorted);
    true -> {[], Sorted}
  end,
    
  [dump_frame(Frame) || Frame <- Reply],
  put(frame_buffer, Save),
  {Reader1, length(Frames)}.
...
dump_avc(<<>>, Acc) ->
  ...
  Dump = "  " ++ [io_lib:format("NAL(~p) ~p, ", [h264:type(NAL), size(NAL)]) || NAL <- NALS] ++ "~n",
  io:format(Dump);
...
dump_frame(#video_frame{flavor = Flavor, codec = Codec, dts = DTS, pts = PTS, body = Body}) ->
  io:format("-->  ~p(~p): ~p ~p(~p)~n", [Codec, Flavor, round(DTS), round(PTS - DTS), iolist_size(Body)]),
  ok.
